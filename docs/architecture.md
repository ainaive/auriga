...
| [`@auriga/provider`](../packages/provider) | Model-provider seam + the Anthropic, OpenAI, Gemini, and Bedrock implementations plus OpenAI-compatible gateways (DeepSeek, Bailian, Moonshot, Zhipu) via the `OPENAI_COMPATIBLE` registry, a deterministic stub, a `providerFor(modelId)` factory that selects the backend from the model-id prefix (or an explicit `vendor/model` override), and model routing (the "reasoning sandwich"). |
...
| `ModelProvider` | [`core/src/provider/types.ts`](../packages/core/src/provider/types.ts) | `AnthropicProvider`, `OpenAIProvider` (also backs the OpenAI-compatible gateways), `GeminiProvider`, `BedrockProvider`, `StubProvider`, `ReplayProvider` (chosen per model id by `providerFor`) | glued (SDK) |
