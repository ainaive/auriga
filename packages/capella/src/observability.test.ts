...
test("Recorder collects events and seals a trace", () => {
  ...
  rec.record({
    type: "model_response",
    step: 2,
    response: textResponse("done", { usage: { input_tokens: 7, output_tokens: 3 } }),
  });
...
test("traceCost rolls up tokens + cost from model_response events", () => {
  ...
  rec.record({
    type: "model_response",
    step: 1,
    response: textResponse("a", { usage: { input_tokens: 1_000_000, output_tokens: 0 } }),
  });
...
test("formatTrace renders events and a cost footer", () => {
  ...
  rec.record({
    type: "model_response",
    step: 1,
    response: textResponse("done", { usage: { input_tokens: 10, output_tokens: 5 } }),
  });
