...
test("estimateCostUsd uses per-million pricing", () => {
  ...
  const cost = estimateCostUsd("claude-sonnet-4-6", { input_tokens: 1_000_000, output_tokens: 1_000_000 });
...
test("formatUsage shows tokens and cost, or n/a without a model", () => {
  expect(formatUsage("claude-haiku-4-5-20251001", { input_tokens: 1000, output_tokens: 0 })).toContain(
    "in=1000",
  );
